...
import project.logarithms.Ln
import project.logarithms.Log
...
class Function(
    ...
    fun calc(x: Double): Double {
        return if (x <= 0.0) (((sn.calc(x) * ct.calc(x) * sn.calc(x) / (sn.calc(x) * ct.calc(x))) +
                    (tn.calc(x) * sc.calc(x) / sc.calc(x))) * (sc.calc(x) - sn.calc(x)))
        else (((lg.calc(x, 10.0) + lg.calc(x, 10.0)).pow(3) + lg.calc(x, 10.0)) /
